...
class ElectrodeArray:
    ...
    def __init__(
        ...
        self.xt = self.extract_area6v(xt_in, start_chan, end_chan)
        self.num_samples = self.xt.shape[0] if self.xt is not None else 0
    ...
    def extract_area6v(self, xt, start_chan, end_chan):
        ...
        if xt is not None:
            extracted_xt = xt[:, start_chan:end_chan]
        else:
            extracted_xt = None
    ...
    def implot(self, time_start, time_end, time_step=1, interval=100):
        ...
        if data.size != num_frames * self.num_rows * self.num_cols:
            raise ValueError(
                f"Data size mismatch: expected {expected_size}, " f"got {data.size}"
            )
        ...
        fig = px.imshow(
            ...
            aspect="equal",
            labels=dict(
                animation_frame="Time Step", x="X-axis", y="Y-axis", color=self.desc
            ),
        )

        fig.update_layout(
            title=f"Array: {self.idkey}", xaxis_title="X-axis", yaxis_title="Y-axis"
        )

        # Safely set animation speed
        if fig.layout.updatemenus and fig.layout.updatemenus[0].buttons:
            fig.layout.updatemenus[0].buttons[0].args[1]["frame"]["duration"] = interval
        ...
        try:
            with open(fname, "w") as f:
                ...
                if self.xt is not None and self.xt.size > 0:
                    ...
                    print(
                        f"Warning: self.xt is empty for {self.idkey}, not saving data."
                    )
