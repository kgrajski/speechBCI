...
def compare_input_formats():
    ...
    one_hot_projector = EmbeddingProjector(
        input_dim=codebook_size,  # Large input dimension for one-hot
        model_dim=512
    )
    ...
    print(f"One-hot input shape: {one_hot.shape}, "
          f"Memory: {one_hot.element_size() * one_hot.nelement() / 1024 / 1024:.2f} MB")
    print(f"Codebook vector input shape: {codebook_vectors.shape}, "
    ...
    print(f"Parameter reduction using codebook vectors: {(1 - codebook_params/one_hot_params)*100:.1f}%")
...
def prepare_vqvae_inputs(indices, codebook, use_recommended_method=True):
    ...
    if use_recommended_method:
        ...
        print("WARNING: Using one-hot vectors is not recommended due to memory and " 
