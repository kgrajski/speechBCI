...
class ModelDiagnostics:
    ...
    - Mode collapse/compression (when encoder outputs become too similar)
    ...
    def __init__(self, model, encoder, tokenizer, writer, output_dir):
        """
        Initialize the diagnostics module

        Args:
            model: The base MMLLM model
            encoder: The input encoder component
    ...
    def _analyze_encoder_outputs(self, encoder_outputs, epoch):
        """
        Analyze the encoder outputs for diversity and mode collapse patterns
        ...
        """
        # Basic statistics
        mean_val = encoder_outputs.mean().item()
        std_val = encoder_outputs.std().item()

        # Check for mode collapse
        batch_size = encoder_outputs.shape[0]
        if batch_size > 1:
            # Flatten each example
            flat_outputs = encoder_outputs.view(batch_size, -1)
            ...
            mask = torch.ones_like(similarity) - torch.eye(
                similarity.size(0), device=similarity.device
            )
            ...
            self.output_diversity.append(
                {"epoch": epoch, "mean_sim": mean_sim.item(), "std": std_val}
            )
    ...
    def _analyze_text_diversity(self, decoded_texts, epoch):
        ...
        self.writer.add_scalar(
            "Text/unique_ratio", unique_texts / len(decoded_texts), epoch
        )
        ...
        for att_data in self.attention_patterns:
            ...
            for layer_idx, layer_weights in enumerate(weights):
                ...
                if len(layer_weights.shape) == 4:
                    ...
                    print(
                        f"Skipping layer with unexpected shape: {layer_weights.shape}"
                    )
                    continue

                # Create attention heatmap for first example in batch, first head
                fig = go.Figure(
                    data=go.Heatmap(z=layer_weights[0, 0], colorscale="Viridis")
                )
    ...
    def analyze_mode_collapse(self):
        ...
        Mode collapse is when the encoder maps different inputs to very similar
        ...
        fig.add_trace(
            go.Scatter(
                x=epochs, y=stds, mode="lines+markers", name="Output Standard Deviation"
            ),
            row=1,
            col=2,
        )
        ...
        # Update layout
        fig.update_layout(
            title="Mode Collapse Analysis Over Training", height=500, width=1200
        )
        ...
        with open(os.path.join(self.output_dir, "mode_collapse_summary.txt"), "w") as f:
            ...
            if similarities[-1] > 0.8:
                f.write(
                    "RECOMMENDATION: Increase diversity_loss_weight to combat mode collapse\n"
                )
                f.write(
                    "RECOMMENDATION: Try lower learning rate or different attention mechanisms\n"
                )
            else:
    ...
    def capture_epoch_diagnostics(self, model, dataloader, epoch):
        ...
        with torch.no_grad():
            ...
            # Forward pass through MultimodalLLM
            encoder_outputs, mmllm_outputs = model(
            ...
            attention_weights = []
            if hasattr(mmllm_outputs, "encoder_attentions") and mmllm_outputs.encoder_attentions:
                attention_weights.extend(
                    [attn.cpu() for attn in mmllm_outputs.encoder_attentions]
                )
            if hasattr(mmllm_outputs, "decoder_attentions") and mmllm_outputs.decoder_attentions:
                attention_weights.extend(
                    [attn.cpu() for attn in mmllm_outputs.decoder_attentions]
                )
            if hasattr(mmllm_outputs, "cross_attentions") and mmllm_outputs.cross_attentions:
                attention_weights.extend(
                    [attn.cpu() for attn in mmllm_outputs.cross_attentions]
                )

            # Analyze encoder outputs
            self._analyze_encoder_outputs(encoder_outputs, epoch)

            # Store attention patterns for later visualization
            if attention_weights:
                self.attention_patterns.append(
                    {"epoch": epoch, "weights": attention_weights}
                )
        ...
        print("Done with diagnostics for epoch {epoch}")

    def run_all_diagnostics(self, dataloader, model=None):
        """
        Run comprehensive diagnostics on model and encoder

        This performs a full battery of tests to evaluate the health of the model-encoder system:
        ...
        # Analyze outputs from encoder
        print("\nAnalyzing encoder outputs...")
        with torch.no_grad():
            batch = next(iter(dataloader))
            inputs = batch["vqvae_embeddings"].to(device)
            ...
            fig = self._create_encoder_output_visualization(encoder_outputs)
            fig.write_html(os.path.join(self.output_dir, "encoder_output_stats.html"))

        # Analyze encoder parameters
        if hasattr(self.encoder, "get_diagnostic_data"):
            print("\nCollecting encoder diagnostic data...")
            _, diag_data = self.encoder.get_diagnostic_data(inputs)
            self._visualize_encoder_diagnostics(diag_data)
    ...
    def _create_encoder_output_visualization(self, encoder_outputs):
        """
        Create visualization of encoder output statistics
        ...
        Args:
            encoder_outputs: Tensor of encoder outputs

        Returns:
            Plotly figure with dimension statistics
        """
        # Flatten outputs for analysis
        flat_outputs = encoder_outputs.reshape(-1, encoder_outputs.shape[-1])

        # Calculate statistics
        mean_values = encoder_outputs.mean(dim=[0, 1]).cpu().numpy()
        std_values = encoder_outputs.std(dim=[0, 1]).cpu().numpy()
        ...
        fig = make_subplots(
            ...
            cols=1,
            subplot_titles=[
                "Adapter Output Mean per Dimension",
        ...
        fig.add_trace(
            go.Bar(
                x=list(range(len(mean_values))), y=mean_values, name="Dimension Mean"
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Bar(
                x=list(range(len(std_values))), y=std_values, name="Dimension StdDev"
            ),
            row=2,
            col=1,
        )
        ...
        fig.update_layout(
            height=800, width=1000, title_text="Adapter Output Statistics"
        )
        return fig

    def _visualize_encoder_diagnostics(self, diag_data):
        """
        Visualize encoder's internal behavior based on diagnostic data
        ...
        Args:
            diag_data: Dictionary of diagnostic data from encoder
        """
        # Create directory for encoder visualizations
        encoder_viz_dir = os.path.join(self.output_dir, "encoder_internals")
        os.makedirs(encoder_viz_dir, exist_ok=True)
        ...
        if "layer_outputs" in diag_data and diag_data["layer_outputs"]:
            for i, layer_output in enumerate(diag_data["layer_outputs"]):
                ...
                fig.write_html(
                    os.path.join(encoder_viz_dir, f"layer_{i+1}_activations.html")
                )
        ...
        if "attention_weights" in diag_data and diag_data["attention_weights"]:
            for i, attn_weights in enumerate(diag_data["attention_weights"]):
                ...
                fig.write_html(
                    os.path.join(encoder_viz_dir, f"attention_{i+1}_head_0.html")
                )
    ...
    def plot_umap_projection(self, data, trial_id, n_components=2):
        ...
        fig = px.scatter(
            ...
            x=0,
            y=1,
            labels={"0": "UMAP Dimension 1", "1": "UMAP Dimension 2"},
            title=f"UMAP Projection - Trial: {trial_id}",
        )

        filename = os.path.join(
            self.output_dir, f"umap_projection_trial_{trial_id}.html"
        )
        fig.write_html(filename)
        print(f"Saved UMAP projection to {filename}")
    ...
    def plot_attention_heatmap(self, attention_weights, trial_id, layer_num, head_num):
        ...
        filename = os.path.join(
            self.output_dir,
            f"attention_heatmap_trial_{trial_id}_layer_{layer_num}_head_{head_num}.html",
        )
        fig.write_html(filename)
