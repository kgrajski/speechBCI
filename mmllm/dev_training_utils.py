...
def run_exp(
    ...
    for epoch in range(num_epochs):
        ...
        train_progress = tqdm(train_dl, desc=f"Epoch {epoch+1}/{num_epochs}")
        for batch in train_progress:
            # Process based on data type
            if is_compressed:
                # Compressed data has a different batch format
                input_embeddings = batch["compressed_tokens"].to(device)
                labels = batch["label"]
                ...
                # Original handling for embedded data
                inputs = batch["vqvae_embeddings"].to(device)
                attention_mask = batch["attention_mask"].to(device)
                labels = batch["label_embeddings"].to(device)
            ...
            # Continue with loss calculation and optimization
            loss = outputs.loss
            total_loss += loss.item()
