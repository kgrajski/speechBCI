    ...
    def forward(self, input_embeddings, attention_mask=None, target_texts=None):
        ...
        if target_texts is not None:
            ...
            encoder_outputs = self.t5_model.encoder(
                inputs_embeds=projected_embeddings, attention_mask=attention_mask
            )
...
def train_multimodal_llm(
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=batch_size, collate_fn=variable_length_collate_fn
    )
    ...
    for epoch in range(epochs):
        model.train()
        total_loss = 0
        for batch_idx, (inputs, attention_mask, targets) in enumerate(
...
def get_multimodal_llm(model_type="t5", model_size="small", input_dim=512):
    ...
    if model_type.lower() == "t5":
        ...
        try:
            ...
            print(
                "Warning: LLaMA uses a decoder-only architecture and requires special handling"
            )
        except:
            raise ValueError(
                "LLaMA models require Hugging Face access approval or open-source alternatives"
            )
        ...
        print(
            "Warning: GPT-2 uses a decoder-only architecture and requires special handling"
        )
    ...
    def _forward_decoder_only(self, projected_embeddings, target_texts=None):
        ...
        if target_texts is not None:
            ...
            target_mask = torch.triu(
                torch.ones(target_len, target_len, device=projected_embeddings.device)
                * -1e10,
                diagonal=1,
            )
