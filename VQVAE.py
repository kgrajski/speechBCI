...
# FROM: https://github.com/Vrushank264/VQVAE-PyTorch/blob/main/vqvae.py
#
# Adapted for use in the SpeechBCI project.
#
# The original code was written by Vrushank Majithia.
#
...
class VQVAE(nn.Module):
    ...
    def __init__(self, in_c, out_c, K=32, kernel_size=4, stride=2, padding=1):
        
        super().__init__()
        self.encoder = nn.Sequential(nn.Conv2d(in_c, out_c, kernel_size, stride, padding),
                                     nn.BatchNorm2d(out_c),
                                     nn.ReLU(inplace = True),
                                     nn.Conv2d(out_c, out_c, kernel_size, stride, padding),
                                     ResidualBlock(out_c),
                                     ResidualBlock(out_c)
                                     )
        ...
        self.decoder = nn.Sequential(ResidualBlock(out_c),
                                     ResidualBlock(out_c),
                                     nn.ReLU(inplace = True),
                                     nn.ConvTranspose2d(out_c, out_c, kernel_size, stride, padding),
                                     nn.BatchNorm2d(out_c),
                                     nn.ReLU(inplace = True),
                                     nn.ConvTranspose2d(out_c, in_c, kernel_size, stride, padding),
                                     nn.Tanh()
                                     )
