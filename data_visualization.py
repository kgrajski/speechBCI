...
torch.load = lambda f, *args, **kwargs: _original_load(
    f, map_location=torch.device("cpu"), *args, **kwargs
)
...
def setup_directories():
    ...
    output_dir = os.path.join(
        data_dir, "visualizations", "trajectory_analysis", vqvae_model_name
    )
...
def load_data(embedded_dir, etl_dir, compressed_dir=None):
    ...
    embedded_dataset = SpeechBCIDataSet_Raw(embed_dir=embedded_dir, etl_dir=etl_dir)
    ...
    if compressed_dir and os.path.exists(compressed_dir):
        ...
        compressed_dataset = CompressedBCIDataset(
            compress_dir=compressed_dir, etl_dir=etl_dir
        )
    ...
    return (
        train_embedded_loader,
        test_embedded_loader,
        val_embedded_loader,
        train_compressed_loader,
        test_compressed_loader,
        val_compressed_loader,
        embedded_dataset,
        compressed_dataset,
    )
...
def get_umap_model():
    ...
    return umap.UMAP(n_neighbors=15, min_dist=0.1, n_components=2, random_state=42)
...
def create_embedded_plot(embedded_data, label, trial_id):
    ...
    for i in range(
        0, n_points - 1, max(1, n_points // 10)
    ):  # Add arrows at several points
        fig.add_annotation(
            x=embedding[i + 1, 0],
            y=embedding[i + 1, 1],
            ax=embedding[i, 0],
...
def process_training_data(train_embedded_loader, train_compressed_loader, output_dir):
        ...
        try:
            ...
            if has_compressed:
                ...
                output_path = os.path.join(
                    output_dir, f"{filename_base}_comparison.html"
                )
    ...
    print(
        f"Visualization generation complete! Created {count} visualizations in {output_dir}"
    )
    return count
...
def main():
    ...
    (
        ...
        embedded_dataset,
        compressed_dataset,
    ) = load_data(embedded_dir, etl_dir, compressed_dir)
