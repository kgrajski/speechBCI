...
class SpeechBCITranslator(nn.Module):
    ...
    def __init__(self, vqvae_dim=256, model_name="t5-base", max_length=512):
        ...
        self.projector = VQVAEProjector(
            vqvae_dim=vqvae_dim, llm_dim=self.model.config.d_model, hidden_dim=512
        )
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in progress_bar:
            ...
            attention_mask = torch.ones(
                vqvae_embeddings.shape[0], vqvae_embeddings.shape[1], device=device
            )
            ...
            optimizer.zero_grad()
            outputs = model(
                vqvae_embeddings, attention_mask=attention_mask, labels=labels
            )
            loss = outputs.loss
            ...
            for i in range(len(vqvae_embeddings)):
                if vqvae_embeddings[i].shape[0] > model.max_length:
                    ...
                    prediction = process_long_sequence(
                        model, vqvae_embeddings[i], max_length=model.max_length
                    )
                    ...
                    output_ids = model(
                        vqvae_embeddings[i].unsqueeze(0), attention_mask=attention_mask
                    )
